...
async function getAttendanceTableData(req, res, next) {
  try {
    ...
    for (const transaction of transactions || []) {
      const employeeKey = String(transaction?.emp_code || getEmployeeId(transaction) || "").trim();
    ...
    punchGroups.forEach((group) => {
      ...
      const attendanceStatus =
        derived.punch_count === 1 ? derived.punchAttendanceStatus : ruleResult.attendanceStatus || "P";
